  ...
  return (
    <div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 overflow-hidden">
        {/* Header */}
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-4 border-b border-zinc-800 bg-zinc-900/80 px-4 py-3 text-xs font-medium text-gray-400">
          <div>INDICADOR</div>
          <div>ESTADO</div>
          <div>ORIGEN</div>
          <div>CONCEDIDO</div>
        ...
        <div className="divide-y divide-zinc-800">
          {accesses.map((access) => {
            ...
            return (
              <div
                key={access.id}
                className={`grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-4 items-center px-4 py-3 transition-colors hover:bg-zinc-800/30 ${expired ? 'opacity-60' : ''}`}
              >
                {/* Columna: Indicador */}
