...
export async function POST(
  ...
  try {
    ...
    if (!user || !(await checkAdminPermission(user.id, PERMISSIONS.USERS_REVOKE_ACCESS))) {
      return NextResponse.json({ error: 'No autorizado - Requiere permisos para revocar acceso' }, { status: 401 });
    }
